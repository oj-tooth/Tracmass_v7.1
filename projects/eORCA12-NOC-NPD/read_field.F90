SUBROUTINE read_field
  ...
  ! ==========================================================================
   ...
   CHARACTER (len=200)                      :: fieldFile, dateprefix, eezDataDir, eezFile
   ...
   ! ===========================================================================
   IF (ints == 0) THEN
      PRINT *, 'imindom:', imindom, 'jmindom:', jmindom, 'imt:', imt, 'jmt:', jmt
   ...
   IF (l_tracers) THEN
     ...
     DO itrac = 1, numtracers
        ...
        IF (tracers(itrac)%action == 'read') THEN
            ...
            ! Modification [06-02-2025]: Add EEZ Identifier as tracer variable
            IF (tracers(itrac)%name == 'eez_id') THEN
              eezDataDir = '/work/n01/n01/otooth/mCDR/data/seeding/'
              eezFile = 'eORCA12_EEZ_id_nomask.nc'
              fieldFile = TRIM(eezDataDir)//TRIM(eezFile)
            ELSE
              fieldFile = TRIM(physDataDir)//TRIM(physPrefixForm)//TRIM(dateprefix)//TRIM(tGridName)//TRIM(fileSuffix)
            END IF           
